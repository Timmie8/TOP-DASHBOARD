...
st.markdown("""
    ...
    .alert-trend { border: 2px solid #3fb950 !important; box-shadow: 0 0 10px rgba(63, 185, 80, 0.3); }
    .alert-breakout { border: 2px solid #2563eb !important; box-shadow: 0 0 10px rgba(37, 99, 235, 0.3); }
...
def get_analysis(ticker_symbol):
    try:
        ...
        return {
            ...
            "macd_bull": ml > sl, "ema_ok": price > e20, "change": change
        }
    except: return None
...
for t in st.session_state.watchlist:
    ...
    if res:
        ...
        if res['score'] >= 90: active_alerts.append({"msg": f"🔥 {t}: Extreme Momentum", "type": "warning"})
...
if active_data:
    ...
    with k4: st.markdown(f'<div class="kpi-card"><div class="kpi-label">EMA 20</div><div class="kpi-value">{"OK" if active_data["ema_ok"] else "WEAK"}</div></div>', unsafe_allow_html=True)
    ...
    with col_alerts:
        ...
        if not alert_html: alert_html = '<p style="color:#444; font-size:0.8rem;">No active signals...</p>'
        st.markdown(f'<div class="alert-sidebar">{alert_html}</div>', unsafe_allow_html=True)
        # Popup logic for 20 seconds (hidden sound + timer)
        if active_alerts:
            components.html(f"""
                <audio autoplay><source src="https://assets.mixkit.co/active_storage/sfx/2869/2869-preview.mp3" type="audio/mpeg"></audio>
                <div id="popup" style="position:fixed; top:20px; right:20px; background:#2563eb; color:white; padding:15px; border-radius:10px; z-index:10000; box-shadow:0 0 20px rgba(0,0,0,0.5); font-family:sans-serif;">
                    <b>New Signal Detected!</b><br>Check sidebar.
                </div>
...
for idx, item in enumerate(st.session_state.watchlist):
    ...
    if w:
        ...
        with cols[idx % 3]:
            st.markdown(f'<div class="wl-card {alert_c}"><div style="display:flex;justify-content:space-between"><b>{item}</b><span class="badge badge-{w["signal"].lower()}">{w["signal"]}</span></div><div style="display:flex;justify-content:space-between;margin-top:10px;"><span>${w["price"]:.2f}</span><span class="{sw_c}">Score: {w["score"]}</span></div></div>', unsafe_allow_html=True)
            b1, b2 = st.columns(2)

...
# --- TOOL 1: SMART TERMINAL (ORIGINEEL) ---
tool1_html = """
<!DOCTYPE html>
<html>
<head>
...
<div style="background:#0d1117; color:#c9d1d9; font-family:sans-serif; padding:20px; min-height:800px;">
    <div style="max-width:800px; margin:auto;">
        ...
        <div style="display:flex; gap:10px; margin-bottom:25px;">
            ...
            <button onclick="s2()" style="background:#1f6feb; color:white; border:none; padding:12px 25px; border-radius:8px; font-weight:bold; cursor:pointer;">CALCULATE LEVELS</button>
        </div>
        <div id="out2"></div>
    </div>
</div>
    ...
    document.getElementById('out2').innerHTML = `
        <div style="background:#161b22; border:1px solid #30363d; border-left:8px solid #39d353; padding:25px; border-radius:15px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                <div><b style="font-size:1.5rem; color:white;">${t}</b><br><span style="color:#8b949e;">Asset Tier Analysis</span></div>
                <div style="font-size:2rem; font-weight:900; color:#39d353;">${tier}</div>
            </div>
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:15px; text-align:center;">
                <div style="background:#0d1117; padding:15px; border-radius:10px;"><small style="color:#f85149; font-weight:bold;">STOP LOSS</small><br><b style="font-size:1.2rem;">$${(d.c*0.96).toFixed(2)}</b></div>
                <div style="background:#0d1117; padding:15px; border-radius:10px;"><small style="color:#8b949e; font-weight:bold;">ENTRY</small><br><b style="font-size:1.2rem;">$${d.c.toFixed(2)}</b></div>
                <div style="background:#0d1117; padding:15px; border-radius:10px;"><small style="color:#39d353; font-weight:bold;">TARGET</small><br><b style="font-size:1.2rem;">$${(d.c*1.08).toFixed(2)}</b></div>
            </div>
        </div>`;
}
</script>
...
<head>
    <style>
        ...
        textarea { width: 100%; background: #010409; border: 1px solid #30363d; color: white; padding: 15px; border-radius: 12px; margin-bottom: 10px; height: 60px; outline:none; }
        ...
        .badge { padding: 5px 10px; border-radius: 6px; font-weight: bold; font-size: 12px; color:white; }
...
<body>
    <div class="main-card">
        ...
        <textarea id="listIn" placeholder="AAPL, NVDA, TSLA, MSFT, AMD..."></textarea>
        ...
        async function runScanner() {
            ...
            for(let i=0; i<tickers.length; i++) {
                ...
                try {
                    ...
                    const d = await r.json();
                    if(d.c) { dataList.push({t: t, p: d.c, s: Math.round(50 + (d.dp * 7))}); }
                } catch(e) {}
...
# --- TABS DEFINITIES ---
tab1, tab2, tab3 = st.tabs(["🚀 SMART TERMINAL", "🛡️ RISK & TIER", "📊 PRO SCANNER v5.7"])

...
<div style="background: #0d1117; color: #e6edf3; font-family: sans-serif; padding: 20px;">
    <div style="max-width: 1000px; margin: auto; background: #0d1117; border: 1px solid #30363d; padding: 30px; border-radius: 20px;">
        ...
        <div id="loader" style="display:none; text-align:center; margin:20px; color:#58a6ff;">Analysing market data...</div>
...
# --- TOOL 4: SIGNAL ANALYZER (JOUW CODE) ---
tool4_html = """
<!DOCTYPE html>
<html lang="nl">
<head>
    <style>
        ...
        .search-box { margin-bottom: 14px; display: flex; align-items: center; gap: 8px; }
        input, select { padding: 10px; border-radius: 8px; border: none; background: #1c1c1c; color: #fff; }
        button { padding: 10px 16px; border-radius: 8px; border: none; cursor: pointer; background: #2ecc71; color: #000; font-weight: bold; }
...
async function loadData() {
    ...
    try {
        ...
        tbody.innerHTML = `<tr><td>Signal for ${sym}</td><td class="bullish">SUCCESS: ${last.toFixed(2)}</td></tr>`;
